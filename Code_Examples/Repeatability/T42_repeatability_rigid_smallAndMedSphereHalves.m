...
hand = 'T42';
surface = 'rigid';
object = 'smallAndMedSphereHalves';
motion = {'roty_1'};
%% importing and windowing data 
%setting up the start and end points to be used inorder to remove outliers
minDist = [300];
bestAxis = [4]; %the best axis to use from the data to find endpoints
startClipping = [1700];%array for removing front endpoints of each motion of its best axis 
endClipping = [2800];% array for removing endpoints for this point for each motion of its best axis 
startCycle = [1]; %starting cycles
endCycle = [0]; %cycle points to remove from the end
